...
# Add an A record to the domain for www.example.com.
resource "digitalocean_record" "repo01" {
  domain = digitalocean_domain.habbispw.name
  type   = "A"
  name   = "repo01"
  #value = digitalocean_droplet.repo01.ipv4_address
  value = digitalocean_droplet.proxy01.ipv4_address
}

# Add an A record to the domain for www.example.com.
resource "digitalocean_record" "int" {
  domain = digitalocean_domain.habbispw.name
  type   = "A"
  name   = "int"
...
## snap shot build by packer cloudbast
...
# snap shot build by packer cloudbast
data "digitalocean_image" "ubuntu18_04_docker" {
  name = "ubuntu18_04_docker"
}
...
resource "digitalocean_droplet" "cloudbast01" {
  name     = "cloudbast01"
  size     = "s-1vcpu-1gb"
  image    = data.digitalocean_image.ubuntu18_04lite.image
  region   = "fra1"
  monitoring = "true"
...
resource "digitalocean_droplet" "media01" {
  name     = "media01"
  size     = "s-1vcpu-1gb"
  image    = data.digitalocean_image.ubuntu18_04_docker.image
  region   = "fra1"
  monitoring = "true"
  vpc_uuid = digitalocean_vpc.fra2-net.id
  ssh_keys = [data.digitalocean_ssh_key.ssh_keys01.id, data.digitalocean_ssh_key.ssh_keys02.id, data.digitalocean_ssh_key.ssh_keys03.id, data.digitalocean_ssh_key.ssh_keys04.id, data.digitalocean_ssh_key.ssh_keys05.id ]
...
resource "digitalocean_firewall" "media" {
  ...
  droplet_ids = [digitalocean_droplet.repo01.id, digitalocean_droplet.wiki.id, digitalocean_droplet.media01.id]
